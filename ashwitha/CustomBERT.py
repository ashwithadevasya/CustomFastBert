...
class MyBertLearner(Learner):
    ...
    def __init__(
        ...
        super(MyBertLearner, self).__init__(
            data,
            model,
            pretrained_model_path,
            output_dir,
    ...
    def predict_batch(self, texts=None):
        ...
        results = self.predict_batch_df(texts).to_dict("record")
        return [sorted(x.items(), key=lambda kv: kv[1], reverse=True) for x in results]

    ### Return Predictions ###
    def predict_batch_df(self, texts=None, threshold = 0, label_cols = []):
        ...
        ### Return Predictions ###
    def predict_with_filter(self, texts=None, threshold=0, label_cols=[]):
        result_df = self.predict_batch_df(texts, threshold, label_cols)
